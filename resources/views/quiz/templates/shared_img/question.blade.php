...
@if(! empty($imageThemePartial))
    @include($imageThemePartial, ['templateScreen' => $templateScreen ?? 'question'])
@endif
...
@php
    ...
    $compactAnswerLayout = false;
    if ($adaptiveAnswerLayout ?? false) {
        $answerTextMetrics = $question->answers->map(function ($answer): array {
            $plainText = trim(strip_tags((string) $answer->text));
        ...
        $compactCharacterLimit = $question->image ? 30 : 42;
        $compactWordLimit = $question->image ? 5 : 7;
        $compactAnswerLayout = $question->answers->count() <= 6
            && (int) $answerTextMetrics->max('characters') <= $compactCharacterLimit
            && (int) $answerTextMetrics->max('words') <= $compactWordLimit;
    }
@endphp
            ...
            <div class="question-stage{{ $question->image ? ' question-stage--with-media' : '' }}">
                ...
                <div class="question-content-column">
                    ...
                    <form id="quiz-form"
                        ...
                        <div class="answer-list{{ ($adaptiveAnswerLayout ?? false) ? ($compactAnswerLayout ? ' image-answer-list--compact' : ' image-answer-list--comfortable') : '' }}"
                             @if($adaptiveAnswerLayout ?? false) data-answer-layout="{{ $compactAnswerLayout ? 'compact' : 'comfortable' }}" @endif>
                ...
                <div class="selection-state question-stage__selection" role="status" aria-live="polite" aria-atomic="true">
